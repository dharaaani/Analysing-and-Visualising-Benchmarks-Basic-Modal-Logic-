...
def insert_pca_data_from_csv(file_path):
    ...
    print(df.head())  # Debugging: Print to verify the data
    ...
    insert_query = """
    INSERT INTO pca (PC1, PC2, Problem, Prover, System_name)
    VALUES (%s, %s, %s, %s, %s)
    """

    for _, row in df.iterrows():
        row_values = (row['PC1'], row['PC2'], row['Problem'], row['Prover'], row['System'])
        print(f"Attempting to insert row: {row_values}")  # Debugging statement
        try:
            cursor.execute(insert_query, row_values)
            print(f"Inserted row: {row_values}")  # Confirm insertion
        except mysql.connector.Error as err:
...
def preprocess_dataset(file_path):
    ...
    # Generate the selected_100.csv as part of the preprocessing
    output_file = file_path.replace(".csv", "_output.csv")
    selected_100_file = file_path.replace(".csv", "_selected_100.csv")
    

    print(f"Selected 100 rows saved as '{selected_100_file}'")

    # Insert output.csv data into measurements table
    # Your existing insertion logic for measurements

    # Now insert the selected_100.csv data into the pca table
    insert_pca_data_from_csv(selected_100_file)
